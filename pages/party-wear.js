...
const Mugs = ({ products }) => {
  ...
  return (
    <div>
      <h2 className="text-center my-5">Party_wear</h2>
      <section className="text-gray-600 ">
        <div className=" px-5  mx-auto">
          {Object.keys(products).length === 0 && (
            <h6 className="text-center font-normal h-[50vh]">
              Sorry! No party_wear available. Stay Tuned with us for new
              collection.
            </h6>
          )}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "party_wear" });
  let party_wear = {};
  for (let item of products) {
    if (item.title in party_wear) {
      if (
        !party_wear[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        party_wear[item.title].color.push(item.color);
      }
      if (
        !party_wear[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        party_wear[item.title].size.push(item.size);
      }
    } else {
      party_wear[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        party_wear[item.title].color = [item.color];
        party_wear[item.title].size = [item.size];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(party_wear)) }, // will be passed to the page component as props
  };
}
