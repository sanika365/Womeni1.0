...
const Tshirts = ({ products }) => {
  ...
  return (
    <div>
      <h2 className="text-center my-5">New_In</h2>
      <section className="text-gray-600 ">
        <div className=" px-5  mx-auto">
          {Object.keys(products).length === 0 && (
            <h6 className="text-center font-normal h-[50vh]">
              Sorry! No New_In available. Stay Tuned with us for new
              collection.
            </h6>
          )}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "New_In" });
  let New_In = {};
  for (let item of products) {
    if (item.title in New_In) {
      if (
        !New_In[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        New_In[item.title].color.push(item.color);
      }
      if (
        New_In[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        New_In[item.title].size.push(item.size);
      }
    } else {
      New_In[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        New_In[item.title].color = [item.color];
        New_In[item.title].size = [item.size];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(New_In)) }, // will be passed to the page component as props
  };
}
