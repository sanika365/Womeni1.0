...
const Stickers = ({ products }) => {
  ...
  return (
    <div>
      <h2 className="text-center my-5">Dresses</h2>
      <section className="text-gray-600 ">
        <div className=" px-5  mx-auto">
          {Object.keys(products).length === 0 && (
            <h6 className="text-center font-normal h-[50vh]">
              Sorry! No Dresses available. Stay Tuned with us for new
              collection.
            </h6>
          )}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "Dresses" });
  let Dresses = {};
  for (let item of products) {
    if (item.title in Dresses) {
      if (
        !Dresses[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        Dresses[item.title].color.push(item.color);
      }
      if (
        !Dresses[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        Dresses[item.title].size.push(item.size);
      }
    } else {
      Dresses[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        Dresses[item.title].color = [item.color];
  ...
  return {
    props: { products: JSON.parse(JSON.stringify(Dresses)) }, // will be passed to the page component as props
  };
}
