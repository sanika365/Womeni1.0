...
const Hoodies = ({ products }) => {
  ...
  return (
    <div>
      <h2 className="text-center my-5">Tops</h2>
      <section className="text-gray-600 ">
        <div className=" px-5  mx-auto">
          {Object.keys(products).length === 0 && (
            <h6 className="text-center font-normal h-[50vh]">
              Sorry! No Tops available. Stay Tuned with us for new
              collection.
            </h6>
          )}
...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in Tops) {
      if (
        !Tops[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        Tops[item.title].color.push(item.color);
      }
      if (
        !Tops[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
  ...
  return {
    props: { products: JSON.parse(JSON.stringify(Tops)) }, // will be passed to the page component as props
  };
}
